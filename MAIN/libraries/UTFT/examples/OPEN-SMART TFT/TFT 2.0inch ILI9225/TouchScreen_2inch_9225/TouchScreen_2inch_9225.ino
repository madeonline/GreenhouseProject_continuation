...
#define MINPRESSURE 130
#define MAXPRESSURE 1000
// For better pressure precision, we need to know the resistance
// between X+ and X- Use any multimeter to read it
// For the one we're using, its 550 ohms across the X plate
...
//  tft.fillRect(BOXSIZE*5, 0, BOXSIZE, BOXSIZE, MAGENTA);
  ...
  currentcolor = RED;
tft.print("OPEN-SMART TFT", CENTER, 150);
}
int erase = 0;
void loop(void) {
  ...
  if (p.z > MINPRESSURE && p.z < MAXPRESSURE) {
	...
	//tft.fillScr(BLACK);
	 int16_t temp = p.x;
  	 p.y = map(p.y, TS_MINY, TS_MAXY, 0, 176);
     p.x = map(temp, TS_MINX, TS_MAXX, 220, 0);
    ...
    if (((p.y-PENRADIUS) > BOXSIZE) && ((p.y+PENRADIUS) < 220)) {
