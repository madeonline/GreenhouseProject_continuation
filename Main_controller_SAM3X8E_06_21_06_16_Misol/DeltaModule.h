...
typedef struct
{
  uint16_t SensorType; // тип сенсора
...
//  GlobalSettings* settings; // указатель на настройки
  ...
  static void OnDeltaRead(uint16_t& sensorType, String& moduleName1,uint8_t& sensorIdx1, String& moduleName2, uint8_t& sensorIdx2); // нам передали прочитанные из EEPROM данные одной дельты
  static void OnDeltaGetCount(uint8_t& count); // у нас запросили - сколько установок дельт писать в EEPROM
  static void OnDeltaWrite(uint16_t& sensorType, String& moduleName1,uint8_t& sensorIdx1, String& moduleName2, uint8_t& sensorIdx2); // мы передаём данные очередной дельты
