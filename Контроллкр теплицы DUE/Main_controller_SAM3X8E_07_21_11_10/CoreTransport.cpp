...
void CoreESPTransport::sendCommand(ESPCommands command)
{
  ...
  switch(command)
  {
    ...
    {
      #ifdef WIFI_DEBUG
        SerialUSB.println(F("ESP: Starting TCP-server..."));
      #endif
...
{
  SerialUSB.print(F("SIM800 RECEIVE BUFFER, SIZE="));
  SerialUSB.println(receiveBuffer.size());
  
  for(size_t i=0;i<receiveBuffer.size();i++)
  {
    SerialUSB.print(WorkStatus::ToHex(receiveBuffer[i]));
    SerialUSB.print(" ");
  }
...
void CoreSIM800Transport::power(bool on)
{
  ...
  if(gbnd.RebootPinLinkType != linkUnbinded && gbnd.RebootPin != UNBINDED_PIN)
  {
      if(gbnd.RebootPinLinkType == linkDirect)
      {
          ...
          #else
          if(gbnd.RebootPin > 1) // prevent SerialUSB locking
          #endif
          {
...
#ifdef GSM_DEBUG_MODE
	SerialUSB.print("+++ ");
	SerialUSB.print(String(queuedSMS.isFlash));
	SerialUSB.print(" ");
	SerialUSB.print(phoneNumber.c_str());
	SerialUSB.print(" ");
