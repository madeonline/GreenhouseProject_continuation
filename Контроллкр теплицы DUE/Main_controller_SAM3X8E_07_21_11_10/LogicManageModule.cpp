...
void Thermostat::setup(uint8_t _channel)
{
  ...
  SerialUSB.print(F("THERMOSTAT SETUP: CHANNEL #"));
...
void Thermostat::setState(bool on)
{
  ...
  #ifdef THERMOSTAT_DEBUG
  SerialUSB.print(F("THERMOSTAT SET STATE, CHANNEL #"));
...
{
  #ifdef THERMOSTAT_DEBUG
  SerialUSB.print(F("THERMOSTAT RELOAD SETTINGS: CHANNEL #"));
...
{
  #ifdef THERMOSTAT_DEBUG
  SerialUSB.print(F("THERMOSTAT CHECK WANT ON: CHANNEL #"));
  SerialUSB.println(channel);
  #endif
  ...
  {
      #ifdef THERMOSTAT_DEBUG
        SerialUSB.print(F("THERMOSTAT: NO SENSOR TEMPERATURE, CHANNEL #"));
        SerialUSB.println(channel);
      #endif
  ...
  if(tVal >= (tOnBorder + 10l*settings.histeresis))
  {
    ...
    #ifdef THERMOSTAT_DEBUG
    SerialUSB.print(F("THERMOSTAT: CHANNEL #"));
    SerialUSB.print(channel);
    SerialUSB.println(F(" WANTS OFF!"));
    #endif
...
{
  #ifdef VENT_DEBUG
  SerialUSB.print(F("VENT SETUP: CHANNEL #"));
  SerialUSB.println(_channel);
  #endif
...
void Vent::setState(bool on)
{
  ...
  #ifdef VENT_DEBUG
  SerialUSB.print(F("VENT SET STATE, CHANNEL #"));
  SerialUSB.print(channel);
  SerialUSB.print(F("; STATE = "));
  SerialUSB.println(on);
  #endif
...
{
  #ifdef VENT_DEBUG
  SerialUSB.print(F("VENT RELOAD SETTINGS: CHANNEL #"));
  SerialUSB.println(channel);
  #endif
  ...
  switch(machineState)
  {
    ...
    case ventCheckOff:
    {
      ...
      if(millis() - workStartedAt > maxWorkTime)
      {
        ...
        #ifdef VENT_DEBUG
        SerialUSB.print(F("VENT WANTS REST: CHANNEL #"));
        SerialUSB.println(channel);
        #endif
      ...
      if(millis() - tempTimer > VENT_UPDATE_INTERVAL)
      {
        ...
        if(temp.Value == NO_TEMPERATURE_DATA)
        {
          ...
          #ifdef VENT_DEBUG
            SerialUSB.print(F("VENT: NO SENSOR TEMPERATURE, CHANNEL #"));
            SerialUSB.println(channel);
          #endif
        ...
        else
        {
          ...
          if(tVal <= tOffBorder)
          {
            ...
            #ifdef VENT_DEBUG
            SerialUSB.print(F("VENT WANT OFF: CHANNEL #"));
            SerialUSB.println(channel);
            #endif
            ...
            if(millis() - workStartedAt > minWorkTime)
            {
              ...
              #ifdef VENT_DEBUG
              SerialUSB.print(F("VENT OFF: CHANNEL #"));
              SerialUSB.println(channel);
              #endif
            ...
            else
            {
              ...
              #ifdef VENT_DEBUG
              SerialUSB.print(F("VENT NEED ADDITIONAL TIME: CHANNEL #"));
              SerialUSB.print(channel);
              SerialUSB.print(F(", TIME = "));
              SerialUSB.println(addInterval);
              #endif            
    ...
    case ventWaitAddInterval:
    {
      ...
      if(millis() - workStartedAt > addInterval)
      {
        // интервал прошёл
        #ifdef VENT_DEBUG
        SerialUSB.print(F("VENT ADDITIONAL TIME DONE: CHANNEL #"));
        SerialUSB.println(channel);
        #endif
    ...
    case ventRest:
    {
      ...
      if(millis() - workStartedAt > restTime)
      {
        ...
        #ifdef VENT_DEBUG
        SerialUSB.print(F("VENT REST DONE: CHANNEL #"));
        SerialUSB.println(channel);
        #endif
...
{
  #ifdef VENT_DEBUG
  SerialUSB.print(F("VENT CHECK WANT ON: CHANNEL #"));
  SerialUSB.println(channel);
  #endif
  ...
  if(tVal >= tOnBorder)
  {
    // мы превысили порог включения, надо включать !!!
    #ifdef VENT_DEBUG
    SerialUSB.print(F("VENT: CHANNEL #"));
    SerialUSB.print(channel);
    SerialUSB.println(F(" WANTS ON!"));
    #endif
...
void HumiditySpray::setup(uint8_t _channel)
{
  ...
  SerialUSB.print(F("SPRAY SETUP: CHANNEL #"));
  SerialUSB.println(_channel);
  #endif
...
void HumiditySpray::setState(bool on)
{
  ...
  #ifdef HUMIDITY_SPRAY_DEBUG
  SerialUSB.print(F("SPRAY SET STATE, CHANNEL #"));
  SerialUSB.print(channel);
  SerialUSB.print(F("; STATE = "));
  SerialUSB.println(on);
  #endif
...
{
  #ifdef HUMIDITY_SPRAY_DEBUG
  SerialUSB.print(F("SPRAY RELOAD SETTINGS: CHANNEL #"));
  SerialUSB.println(channel);
  #endif
  ...
  switch(machineState)
  {
    case hsmIdle: // в режиме ожидания
    {
      if(canWork()) // можем работать в этом диапазоне
      {
          ...
          {
              #ifdef HUMIDITY_SPRAY_DEBUG
                SerialUSB.print(F("SPRAY: NO SENSOR DATA, CHANNEL #"));
                SerialUSB.println(channel);
              #endif
           ...
           // получили влажность, проверяем уставки
            ...
            if(tVal <= tOnBorder)
            {
              ...
              #ifdef HUMIDITY_SPRAY_DEBUG
              SerialUSB.print(F("SPRAY: CHANNEL #"));
              SerialUSB.print(channel);
              SerialUSB.println(F(" WANTS ON!"));
              #endif
      ...
      // тут проверяем уставку на выключение
        ...
        else
        {
          ...
          if(tVal >= tOffBorder)
          {
            // достигли нижнего порога, надо выключать!
            #ifdef HUMIDITY_SPRAY_DEBUG
            SerialUSB.print(F("SPRAY WANT OFF: CHANNEL #"));
            SerialUSB.println(channel);
            #endif
...
// настраиваем выхода
  ...
  SerialUSB.print("CYCLE VENT CHANNEL #");
  SerialUSB.print(channel);
  SerialUSB.println(" CycleVent::setup()");
  #endif
}
//--------------------------------------------------------------------------------------------------------------------------------------
void CycleVent::on()
{
  ...
  SerialUSB.print("CYCLE VENT CHANNEL #");
  SerialUSB.print(channel);
  SerialUSB.println(" CycleVent::on()");
  #endif
...
void CycleVent::off()
{
  ...
  SerialUSB.print("CYCLE VENT CHANNEL #");
  SerialUSB.print(channel);
  SerialUSB.println(" CycleVent::off()");
  #endif
...
void CycleVent::disable()
{
  ...
  SerialUSB.print("CYCLE VENT CHANNEL #");
  SerialUSB.print(channel);
  SerialUSB.println(" CycleVent::disable()");
  #endif
...
void CycleVent::enable()
{
  ...
  SerialUSB.print("CYCLE VENT CHANNEL #");
  SerialUSB.print(channel);
  SerialUSB.println(" CycleVent::enable()");  
  #endif
  ...
  switch(machineState)
  {
    case cvmIdle:
    {
      ...
      if(canWork(sett))
      {
        ...
        SerialUSB.print("CYCLE VENT CHANNEL #");
        SerialUSB.print(channel);
        SerialUSB.println(" WANTS ON FROM  cvmIdle!");
        #endif
...
void Shadow::on()
{
	...
	if(diff < workTime)
		...
		SerialUSB.print("SHADOW wants ON, TIMER interval: drive #");
		SerialUSB.print(channel);
		SerialUSB.print(", interval=");
		SerialUSB.println(timerInterval);
...
void Shadow::off()
{
	...
	#ifdef SHADOW_MODULE_DEBUG	
		SerialUSB.print("Shadow wants OFF, TIMER interval: drive #");
		SerialUSB.print(channel);
		SerialUSB.print(", interval=");
		SerialUSB.println(timerInterval);
	#endif // SHADOW_MODULE_DEBUG
...
void LogicManageModuleClass::makeShadowDecision(Shadow& shadow, ShadowSettings& section, ShadowDriveCommand& decision)
{
	...
	#ifdef SHADOW_MODULE_DEBUG
		SerialUSB.print("Section lux: ");
		SerialUSB.println(sectionLux);
	#endif // SHADOW_MODULE_DEBUG
	...
	{
		#ifdef SHADOW_MODULE_DEBUG
			SerialUSB.println("NO SECTION LUX!!!");
		#endif // SHADOW_MODULE_DEBUG
	...
	{
		#ifdef SHADOW_MODULE_DEBUG
			SerialUSB.println("WANT SHADOW ON!!!");
		#endif // SHADOW_MODULE_DEBUG
	...
	{
		#ifdef SHADOW_MODULE_DEBUG
			SerialUSB.println("WANT SHADOW OFF!!!");
		#endif // SHADOW_MODULE_DEBUG
...
{	
	...
	#ifdef SHADOW_MODULE_DEBUG
		SerialUSB.println("MAKE SHADOW DECISION!");
	#endif // SHADOW_MODULE_DEBUG
  ...
  if(onMyWay)
  {
      ...
      if(!timerCounter)
      {
        // закончили движение за установленное время
          ...
          SerialUSB.print("Heater #");
          SerialUSB.print(channel);
          SerialUSB.print(" VALVE DONE, position is: ");
          SerialUSB.println(currentPosition);
        ...
        {
          #ifdef HEAT_MODULE_DEBUG
            SerialUSB.println("Heater::update(), valve closed, turn pump OFF.");
          #endif // HEAT_MODULE_DEBUG   
        ...
        {
          #ifdef HEAT_MODULE_DEBUG
            SerialUSB.println("Heater::update(), valve opened, turn pump ON.");
          #endif // HEAT_MODULE_DEBUG
...
void Heater::open(uint8_t percents)
{
  ...
  #ifdef HEAT_MODULE_DEBUG
    SerialUSB.print(channel);
    SerialUSB.print(" Heater::open(), requested position: %");
    SerialUSB.println(percents);
  #endif // HEAT_MODULE_DEBUG
  */
...
/*
  #ifdef HEAT_MODULE_DEBUG
    SerialUSB.print(channel);
    SerialUSB.println(" Heater::open(), SAME POSITION!");
  #endif // HEAT_MODULE_DEBUG
*/
    ...
    if(!currentPosition)
    {
      ...
      #ifdef HEAT_MODULE_DEBUG
        SerialUSB.print(channel);
        SerialUSB.println(" Heater::open(), same position, valve closed, turn pump OFF.");
      #endif // HEAT_MODULE_DEBUG   
      */
    ...
    else
    {
      ...
      #ifdef HEAT_MODULE_DEBUG
        SerialUSB.print(channel);
        SerialUSB.println(" Heater::open(), same position, valve opened, turn pump ON.");
      #endif // HEAT_MODULE_DEBUG
      */
  ...
  if(diff > 0)
  {
    ...
    #ifdef HEAT_MODULE_DEBUG
      if(!onMyWay) // чтобы не спамило в SerialUSB
      {
        SerialUSB.print(channel);
        SerialUSB.print(" Heater::open(), want to CLOSE for ms: ");
        SerialUSB.println(timerCounter);
      }
    #endif // HEAT_MODULE_DEBUG  
  ...
  else
  {
    // текущая позиция меньше запрошенной, надо открыться
      ...
      {
        SerialUSB.print(channel);
        SerialUSB.print(" Heater::open(), want to OPEN for ms: ");
        SerialUSB.println(timerCounter);
      }
    #endif // HEAT_MODULE_DEBUG
...
void LogicManageModuleClass::makeHeatDecision(Heater& heater, HeatSettings& section, HeatDriveCommand& decision, uint8_t& decisionPercents, float& lastI)
{
  ...
  #ifdef HEAT_MODULE_DEBUG
    SerialUSB.print("Section temperature: ");
    SerialUSB.println(sectionTemperature);
    SerialUSB.print("Ambient temperature: ");
    SerialUSB.println(ambientTemperature);
  #endif // HEAT_MODULE_DEBUG
  ...
  {
    #ifdef HEAT_MODULE_DEBUG
      SerialUSB.println("NO SECTION TEMPERATURE!!!");
    #endif // HEAT_MODULE_DEBUG
    decision = hdHeatOff;
  ...
  {
    #ifdef HEAT_MODULE_DEBUG
      SerialUSB.println("SECTION OVERHEAT!!!");
    #endif // HEAT_MODULE_DEBUG
  ...
  if(formattedTemperature <= border)
  {
    ...
    #ifdef HEAT_MODULE_DEBUG
      SerialUSB.println("SECTION FROSEN!!!");
      SerialUSB.print("New percents computed (SECTION FROZEN): ");
      SerialUSB.println(decisionPercents);
    #endif // HEAT_MODULE_DEBUG
  ...
  if(formattedTemperature >= border)
  {
   ...
   #ifdef HEAT_MODULE_DEBUG
      SerialUSB.println("AMBIENT TEMPERATURE IS GOOD :)");
      SerialUSB.print("Percents not changed (AMBIENT IS GOOD): ");
      SerialUSB.println(decisionPercents);
    #endif // HEAT_MODULE_DEBUG
...
//  {
    ...
    #ifdef HEAT_MODULE_DEBUG
      SerialUSB.println("NEED TO HEAT AMBIENT!");
      SerialUSB.print("New percents computed (HEAT AMBIENT): ");
      SerialUSB.println(decisionPercents);
    #endif // HEAT_MODULE_DEBUG
...
{	
	...
	#ifdef HEAT_MODULE_DEBUG
		SerialUSB.println("MAKE HEAT DECISION!");
	#endif // HEAT_MODULE_DEBUG
...
/*
	...
	SerialUSB.print("Want temperature, sensor=");
...
/*
	#if defined(HEAT_MODULE_DEBUG)
	SerialUSB.print("Want temperature, sensorIndex=");
	SerialUSB.print(sensorIndex);
	SerialUSB.print(", module=");
	SerialUSB.println(moduleName);
	#endif
*/
...
{ 
  ...
  if(!_can_work)
  {
	...
	if(windowAutoMode != lastWindowAutoMode)
	{
			...
			SerialUSB.println(F("Windows mode changed, clear windows commands!"));
  ...
  /*
   ...
   if(WindowModule->CanDriveWindows()) // только если можем управлять окнами
   {
  		...
  		if(wantToCloseWindowsBecauseOfHeat)
  		{
  			...
  			#ifdef WINDOW_MANAGE_DEBUG
  			//	SerialUSB.println(F("Want to close ALL windows because of HEAT ON!"));
  			#endif // WINDOW_MANAGE_DEBUG
...
void LogicManageModuleClass::makeWindowsSectionDecision(uint8_t sectionIndex, WManageChannel& section)
{
	...
	{
		#ifdef WINDOW_MANAGE_DEBUG
			SerialUSB.print(F("Section #"));
			SerialUSB.print(sectionIndex);
			SerialUSB.println(F(" has no windows linked, can't make decision!"));
		#endif // WINDOW_MANAGE_DEBUG
	...
	#ifdef WINDOW_MANAGE_DEBUG
		SerialUSB.print(F("Windows for section #"));
		SerialUSB.print(sectionIndex);
		SerialUSB.print(F(" are: "));
		...
		for(size_t i=0;i<ourWindowsNumbers.size();i++)
		{
			...
			SerialUSB.print(ourWindowsNumbers[i]);
		}
		SerialUSB.println();
	#endif // WINDOW_MANAGE_DEBUG
	...
	if(sectionTemperature.HasData())
	{
			...
			SerialUSB.print(F("Section #"));
			SerialUSB.print(sectionIndex);
			SerialUSB.print(F(" temperature is: "));
			SerialUSB.println(sectionTemperature);
		#endif // WINDOW_MANAGE_DEBUG
		...
		int16_t histeresis = section.histeresis*10;
			...
			SerialUSB.print(F("Section #"));
			SerialUSB.print(sectionIndex);
			SerialUSB.print(F(" histeresis are: "));
			SerialUSB.println(histeresis);
		#endif // WINDOW_MANAGE_DEBUG
      ...
      SerialUSB.print(F("Section #"));
      SerialUSB.print(sectionIndex);
      SerialUSB.print(F(" normalized temperature are: "));
      SerialUSB.println(normalizedTemperature);
    #endif // WINDOW_MANAGE_DEBUG      
    ...
    for(size_t i=0;i<ourWindowsNumbers.size();i++)
    {
      ...
      SerialUSB.print(F("Section #"));
      SerialUSB.print(sectionIndex);
      SerialUSB.print(F(" lastTemperatureDecision are: "));
      SerialUSB.println(lastTemperatureDecision);
      ...
      SerialUSB.print(F("Section #"));
      SerialUSB.print(sectionIndex);
      SerialUSB.print(F(" t25percents are: "));
      SerialUSB.println(t25percents);

      SerialUSB.print(F("Section #"));
      SerialUSB.print(sectionIndex);
      SerialUSB.print(F(" t50percents are: "));
      SerialUSB.println(t50percents);

      SerialUSB.print(F("Section #"));
      SerialUSB.print(sectionIndex);
      SerialUSB.print(F(" t75percents are: "));
      SerialUSB.println(t75percents);

      SerialUSB.print(F("Section #"));
      SerialUSB.print(sectionIndex);
      SerialUSB.print(F(" t100percents are: "));
      SerialUSB.println(t100percents);
    ...
    */
    ...
    if(normalizedTemperature >= t100percents)
      temperatureDecision = wdOpen100;               
		...
		#ifdef WINDOW_MANAGE_DEBUG
			SerialUSB.print(F("Temperature decision for section #"));
			SerialUSB.print(sectionIndex);
			SerialUSB.print(F(" are: "));
			SerialUSB.println(temperatureDecision);
		#endif
 ...
 // при этом направление ветра - будет неважно: раз закрыли по порогу и ориентации, то надо ждать снижения ветра.
  ...
  static Vector<size_t> previouslyClosedByHurricaneWindows; // список окон, закрытых по уставке урагана
	...
	if(windSpeedNow >= windSpeedHighBorder) // уставки у нас хранятся в целых, а скорость ветра - измеряется в сотых
	{
		...
		for(size_t i=0;i<ourWindowsNumbers.size();i++)
		{
						...
						if(windOrientation == cpWest)
           {
		...
		#ifdef WINDOW_MANAGE_DEBUG
			SerialUSB.print(F("Wind speed border detected, close section #"));
			SerialUSB.print(sectionIndex);
			SerialUSB.print(F(" linked windows, windows are: "));
			...
			for(size_t i=0;i<orientationCloseWindows.size();i++)
			{
				...
				SerialUSB.print(orientationCloseWindows[i]);
			}
			SerialUSB.println();
		#endif // WINDOW_MANAGE_DEBUG
  ...
  if(windSpeedLowBorder < 0)
	...
	if(windSpeedNow >= windSpeedHighBorder)  // уставки у нас хранятся в целых, а скорость ветра - измеряется в сотых
	{
		...
		#ifdef WINDOW_MANAGE_DEBUG
			SerialUSB.print(F("Hurricane detected, close ALL section #"));
			SerialUSB.print(sectionIndex);
			SerialUSB.println(F(" linked windows!"));
		#endif // WINDOW_MANAGE_DEBUG
  ...
  for(size_t p=0;p<previouslyClosedByHurricaneWindows.size();p++)
  {
	...
	if(hasRain)
	{
		...
		for(size_t i=0;i<ourWindowsNumbers.size();i++)
		{
		...
		#ifdef WINDOW_MANAGE_DEBUG
			SerialUSB.print(F("Rain detected, want to close windows in section #"));
			SerialUSB.print(sectionIndex);
			SerialUSB.print(F(", windows are: "));
			...
			for(size_t i=0;i<rainLinkedWindows.size();i++)
			{
				...
				SerialUSB.print(rainLinkedWindows[i]);
			}
			SerialUSB.println();
		#endif // WINDOW_MANAGE_DEBUG
	...
	// решение по окнам принято, выводим его для отладки
		...
		for(size_t i=0;i<ourWindowsNumbers.size();i++)
		{
			...
			if(windowNumber < windowsDecision.size())
			{
				SerialUSB.print(F("Decision for section #"));
				SerialUSB.print(sectionIndex);
				SerialUSB.print(F(" and window #"));
				SerialUSB.print(windowNumber);
				SerialUSB.print(F(" are: "));
				SerialUSB.println(windowsDecision[windowNumber]);
			}
		} // for	
	#endif // WINDOW_MANAGE_DEBUG
...
void LogicManageModuleClass::makeWindowsDecision()
{
	...
	#ifdef WINDOW_MANAGE_DEBUG
		SerialUSB.println("MAKE WINDOWS DECISION!");
	#endif // WINDOW_MANAGE_DEBUG	
...
void LogicManageModuleClass::applyWindowsDecision()
{
	...
	for(size_t i=0;i<windowsDecision.size();i++)
	{
        ...
        if(window && !window->IsBusy() && !window->IsInUninterruptedWay())
        {
      				...
      				#ifdef WINDOW_MANAGE_DEBUG
      					SerialUSB.print(F("Send command: "));
 ...
 if(bnd.WorkMode == wrsDirect) // датчик прикреплён к контроллеру
 {
    ...
    {    
     ...
     if(bnd.Pin != UNBINDED_PIN)
     {
      ...
      //SerialUSB.println(thisPulses);
      
      //SerialUSB.print("Herz: ");
      //SerialUSB.println(herz);
