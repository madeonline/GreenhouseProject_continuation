...
void SceneModule::AddSceneToList(uint16_t sceneNumber)
{
  ...
  {
    #ifdef SCENE_DEBUG
    SerialUSB.println(F("Scene not found!"));
    #endif
    return;
  }

  #ifdef SCENE_DEBUG
  SerialUSB.println(F("Add scene to list and execute first step..."));
  #endif
...
void SceneModule::ExecuteStep(uint16_t sceneNumber)
{
  ...
  {
    #ifdef SCENE_DEBUG
    SerialUSB.println(F("Scene not found in list!"));
    #endif
    return;
  }
  ...
  #ifdef SCENE_DEBUG
  SerialUSB.print(F("CHECK STEP EXISTS - "));
  SerialUSB.println(dirName);
  #endif
  ...
  {
    #ifdef SCENE_DEBUG
    SerialUSB.println(F("Step NOT EXISTS!"));
    #endif
    
    if(list[sceneIndex].stepNumber == 0 || !list[sceneIndex].continuousMode)
    {
      #ifdef SCENE_DEBUG
      SerialUSB.println(F("SCENE DONE, remove from list!"));
      #endif
      ExecuteSceneFile(sceneNumber, stopFile);
      RemoveSceneFromList(sceneNumber);
    }
    ...
    {
      #ifdef SCENE_DEBUG
      SerialUSB.println(F("Wrap to scene beginning!"));
      #endif
  ...
  #ifdef SCENE_DEBUG
  SerialUSB.print(F("EXECUTE STEP - "));
  SerialUSB.println(dirName);
  #endif
  ...
  if(f.open(dirP,FILE_READ))
  {
    ...
    #ifdef SCENE_DEBUG
    SerialUSB.print(F("Step duration, s = "));
  ...
  {
    #ifdef SCENE_DEBUG
    SerialUSB.println(F("Unable to open step file, skip it!"));
    #endif
...
void SceneModule::ExecuteSceneFile(uint16_t sceneNumber, SceneFile whichFile)
{
  ...
  {
    #ifdef SCENE_DEBUG
    SerialUSB.print(F("Execute scene file - "));
    SerialUSB.println(dirName);
    #endif
