...
void EEPROMSettingsModule::Setup()
{
  ...
  for(uint8_t i=0;i<3;i++)
  {
   ...
   if(!read(WATER_TANK_BINDING_ADDRESS, waterTankBinding))
   {
      ...
      waterTankBinding.DistanceEmpty = 150;
      waterTankBinding.DistanceFull = 50;
   }
     ...
     else
     {
       ...
       if(param == F("WTANK")) // установить настройки модуля контроля бака с водой: CTSET=EES|WTANK|link type|level|max work time|empty_cm|full_cm
       {
          ...
          else
          {
			 ...
			 SerialUSB.print(F("LinkType = "));
			 SerialUSB.println(waterTankBinding.LinkType);
			 SerialUSB.print(F("Level = "));
			 SerialUSB.println(waterTankBinding.Level);
			 SerialUSB.print(F("MaxWorkTime = "));
			 SerialUSB.println(waterTankBinding.MaxWorkTime);
			 SerialUSB.print(F("DistanceEmpty = "));
			 SerialUSB.println(waterTankBinding.DistanceEmpty);
			 SerialUSB.print(F("DistanceFull = "));
			 SerialUSB.println(waterTankBinding.DistanceFull);
#endif 
          }
       } // F("WTANK")       
       #endif // USE_WATER_TANK_MODULE      
