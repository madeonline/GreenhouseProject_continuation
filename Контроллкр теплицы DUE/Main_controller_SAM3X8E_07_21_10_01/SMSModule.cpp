...
{
  #ifdef GSM_DEBUG_MODE
      SerialUSB.print(F("SIM800: INCOMING SMS RECEIVED: "));
  ...
  if(message.startsWith(SMS_RESET_COMMAND))
  {
	...
	//"#2"
	idx = message.indexOf(SMS_TANK_INFO_COMMAND); // отправляем состояние заполнения бака воды
	if (idx != -1)
	{
	#ifdef GSM_DEBUG_MODE
			DEBUG_LOGLN(F("*****TANK INFO ..."));
	#endif

		SendStatToTank(phoneNumber);
		return;
	}

	//"#3"
	idx = message.indexOf(SMS_TANK_ON_COMMAND); // включаем заполнение бака воды
	if (idx != -1)
	{
		#ifdef GSM_DEBUG_MODE
				DEBUG_LOGLN(F("*****TANK ON ..."));
		#endif

		// включаем заполнение бака воды
		ModuleInterop.QueryCommand(ctSET, F("WTANK|FILL|1"), false);
		//SendStatToTank(phoneNumber);
		shouldSendSMS = true;
	}
...
//--------------------------------------------------------------------------------------------------------------------------------
void SMSModule::SendStatToTank(const String& phoneNum)
{
#ifdef GSM_DEBUG_MODE
	DEBUG_LOGLN("Try to send info tank SMS to " + phoneNum + "...");
#endif
	...
	else
	{
		...
		{
			#ifdef GSM_DEBUG_MODE
			SerialUSB.println(F("НЕТ МОДУЛЯ"));
			#endif
			sms = "НЕТ МОДУЛЯ";
		}
		else if (wTankErrorType == 2)
		{
			#ifdef GSM_DEBUG_MODE
			SerialUSB.println(F("СБОЙ НАПОЛНЕНИЯ"));
			#endif
			sms = "СБОЙ НАПОЛНЕНИЯ";
		}
		else if (wTankErrorType == 3)
		{
			#ifdef GSM_DEBUG_MODE
				SerialUSB.println(F("СБОЙ ДАТЧИКА #1"));
			#endif
			sms = "СБОЙ ДАТЧИКА #1";
		}
		else if (wTankErrorType == 4)
		{
			#ifdef GSM_DEBUG_MODE
			SerialUSB.println(F("ОШИБКА"));
			#endif
			sms = "ОШИБКА";
		}

		//switch (wTankErrorType)
		//{
	...
	{
		#ifdef GSM_DEBUG_MODE
		    SerialUSB.print(F("+++++++SMS SEND .. "));
			SerialUSB.println(sms);
		#endif
		SIM800.sendSMS(phoneNum, sms, false);

	}
}
//--------------------------------------------------------------------------------------------------------------------------------
void SMSModule::SendOnToTank(const String& phoneNum)
{
#ifdef GSM_DEBUG_MODE
	DEBUG_LOGLN("Try to send stat tank SMS to " + phoneNum + "...");
#endif
