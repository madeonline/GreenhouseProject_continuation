...
bool CommandParser::ParseCommand(const String& command, Command& outCommand)
{
  ...
  bool rightPrefix = command.startsWith(CMD_PREFIX);//!strncmp_P(readPtr,(const char*)CMD_PREFIX,CMD_PREFIX_LEN);
  ...
  String tmp = String(readPtr);
  bool isGet = tmp.startsWith(CMD_GET);//!strncmp_P(readPtr,(const char*)CMD_GET,CMD_TYPE_LEN);
  bool rightType =  isGet || tmp.startsWith(CMD_SET); //!strncmp_P(readPtr,(const char*)CMD_SET,CMD_TYPE_LEN);
  if(!rightType)
    return false;
