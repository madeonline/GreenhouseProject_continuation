...
#ifdef DS18B20_DEBUG  
  DEBUG_LOG(F("DS18B20: SKIP ROM on pin "));
  DEBUG_LOGLN(String(pin));
 Serial.flush();
#endif 
...
#ifdef DS18B20_DEBUG  
  DEBUG_LOG(F("DS18B20: write scratchpad on pin "));
  DEBUG_LOGLN(String(pin));
 Serial.flush();
#endif
...
#ifdef DS18B20_DEBUG  
  DEBUG_LOG(F("DS18B20: set temp borders on pin "));
  DEBUG_LOGLN(String(pin));
 Serial.flush();
#endif
...
#ifdef DS18B20_DEBUG  
  DEBUG_LOG(F("DS18B20: RESET #1 on pin "));
  DEBUG_LOGLN(String(pin));
 Serial.flush();
#endif
...
#ifdef DS18B20_DEBUG  
  DEBUG_LOG(F("DS18B20: RESET #2 on pin "));
  DEBUG_LOGLN(String(pin));
 Serial.flush();
#endif  
...
#ifdef DS18B20_DEBUG      
       DEBUG_LOG(F("DS18B20 TEMP: "));
       if(result.Negative)
       {
        DEBUG_LOG("-");
       }
       DEBUG_LOG(String(result.Whole));
       DEBUG_LOG(".");
       if(result.Fract < 10)
       {
        DEBUG_LOG("0");
       }
       DEBUG_LOGLN(String(result.Fract));
#endif  
