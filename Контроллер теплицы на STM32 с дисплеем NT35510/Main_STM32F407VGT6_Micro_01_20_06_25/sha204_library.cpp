...
atsha204Class::atsha204Class(uint8_t pin, bool _withDisableInterrupts)
{
  withDisableInterrupts = _withDisableInterrupts;
...
uint8_t atsha204Class::swi_send_bytes(uint8_t count, uint8_t *buffer)
{
  ...
  //Serial.println("swi_send_bytes BEGIN"); Serial.flush();
	// Disable interrupts while sending.

  if(withDisableInterrupts)
  {
  ...
  SHA204_SET_OUTPUT();
	...
	for (i = 0; i < count; i++) 
	{
		...
		{
			if (bit_mask & buffer[i]) // send an "one" bit
			{
				SHA204_POUT_LOW();
	...
	if(withDisableInterrupts)
 {
  ...
  //Serial.println("swi_send_bytes END"); Serial.flush();
