...
public:
  ...
  HTTPQueryProvider* GetHTTPProvider(uint8_t idx) {return httpQueryProviders[idx]; }
  void SetHTTPProvider(uint8_t idx, HTTPQueryProvider* prov) {httpQueryProviders[idx] = prov; }
  ...
  void RemoveAlarm(AlertRule* rule); // обработчик тревог
    ...
    AlarmDispatcher* GetAlarmDispatcher(){ return &alarmDispatcher;}
      ...
      String macChallenge;
      void saveMACChallenge(const String& mac);
      bool testMACChallenge();
    #endif
