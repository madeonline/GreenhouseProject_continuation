...
void TFTSettingsScreen::setup(TFTMenu* menuManager)
{
  ...
  if(!dc)
  {
    ...
    int backButtonHeight = ALL_CHANNELS_BUTTON_HEIGHT;
    backButton = screenButtons->addButton( leftPos ,  screenHeight - backButtonHeight - INFO_BOX_V_SPACING, TFT_BACK_BUTTON_WIDTH,  backButtonHeight, WM_BACK_CAPTION);
    screenButtons->setButtonFont(backButton,TFT_FONT);
    ...
    screenButtons->setButtonFont(timeSyncButton,TFT_FONT);
    ...
    int timeButtonWidth = fontWidth*2+timeButtonPadding*3;
...
/*
    ...
    if(!MainController->checkReg())
    {
      ...
      controlsButtonsTop += buttonHeight + spacing;
      screenButtons->setButtonFontColor(enterKeyButton,RED);
    }
    #endif
*/
...
//      dc->fillRect(rowLeft, rowTop + 2, rowLeft + rowWidth, rowTop + rowHeight - 2);    
      dc->fillRect(rowLeft, rowTop + 2, rowWidth, rowHeight - 2, INFO_BOX_BACK_COLOR);    
...
//      dc->fillRect(maxDataLeft, rowTop + 2, maxDataLeft + maxDataWidth, rowTop + rowHeight - 2);
      dc->fillRect(maxDataLeft, rowTop + 2, maxDataWidth, rowHeight - 2, INFO_BOX_BACK_COLOR);
      yield();        
    }
  }
...
void TFTSensorsScreen::drawListFrame(TFTMenu* menuManager)
{
  ...
  dc->fillRoundRect(270,80,screenWidth - 280, 310,2,INFO_BOX_BACK_COLOR);
...
void TFTWChannelsScreen::setup(TFTMenu* menuManager)
{
  ...
  int controlsButtonsTop = screenHeight - controlButtonHeight - spacing;
    ...
    // Serial.println("TFTWChannelsScreen SETUP DONE!"); Serial.flush();
...
void TFTOrientationScreen::setup(TFTMenu* menuManager)
{
  ...
  // Serial.println("TFTOrientationScreen SETUP DONE!"); Serial.flush();
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void TFTRainScreen::setup(TFTMenu* menuManager)
{
  ...
  // Serial.println("TFTRainScreen SETUP DONE!"); Serial.flush();
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// Serial.println("TFTWindScreen SETUP DONE!"); Serial.flush();
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//    dc->setBackColor(TFT_BACK_COLOR);
    ...
    dc->setTextColor(RED,TFT_BACK_COLOR);
    dc->drawString(which.c_str(), left,top,1);
...
void TFTPHSettingsScreen::drawSensorData(TFTMenu* menuManager, String& which, int left, int top)
{
    ...
    dc->drawString(which.c_str(), left,top);
...
void TFTHeatSettingsScreen::setup(TFTMenu* menuManager)
{
  ...
  if(!dc)
  {
   ...
   airSensorBox = new TFTInfoBox(HEAT_AIR_CAPTION,TFT_TEXT_INPUT_WIDTH,textBoxHeightWithCaption,leftPos,textBoxTopPos,-(TFT_ARROW_BUTTON_WIDTH+INFO_BOX_V_SPACING));
   sensorDataTop2 = textBoxTopPos;
   sensorDataLeft2 = leftPos + rusPrinter->textWidth(HEAT_AIR_CAPTION) + textFontWidth*3 - (TFT_ARROW_BUTTON_WIDTH+INFO_BOX_V_SPACING);
   leftPos += INFO_BOX_V_SPACING + TFT_TEXT_INPUT_WIDTH;
   ...
   sensorBox = new TFTInfoBox(HEAT_SENSOR_CAPTION,TFT_TEXT_INPUT_WIDTH,textBoxHeightWithCaption,leftPos,textBoxTopPos,-(TFT_ARROW_BUTTON_WIDTH+INFO_BOX_V_SPACING));
   sensorDataTop = textBoxTopPos;
   sensorDataLeft = leftPos + rusPrinter->textWidth(HEAT_SENSOR_CAPTION) + textFontWidth*3 - (TFT_ARROW_BUTTON_WIDTH+INFO_BOX_V_SPACING);
   leftPos += INFO_BOX_V_SPACING + TFT_TEXT_INPUT_WIDTH;
...
void TFTIdleScreen::drawWindowStatus(TFTMenu* menuManager)
{
  ...
  #else
    drawValueInBox(windowStatusBox,UNAVAIL_FEATURE,TFT_FONT);
  #endif
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void TFTIdleScreen::drawWaterStatus(TFTMenu* menuManager)
{
  ...
  #else
    drawValueInBox(waterStatusBox,UNAVAIL_FEATURE,TFT_FONT);
  #endif
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void TFTIdleScreen::drawLightStatus(TFTMenu* menuManager)
{
  ...
  #else
    drawValueInBox(lightStatusBox,UNAVAIL_FEATURE,TFT_FONT);
  #endif
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void TFTIdleScreen::drawHeatStatus(TFTMenu* menuManager)
{
  ...
  #else
    drawValueInBox(heatBox1,UNAVAIL_FEATURE,TFT_FONT);
    drawValueInBox(heatBox2,UNAVAIL_FEATURE,TFT_FONT);
    drawValueInBox(heatBox3,UNAVAIL_FEATURE,TFT_FONT);
  #endif // USE_HEAT_MODULE
...
void TFTIdleScreen::drawPHStatus(TFTMenu* menuManager)
{
  ...
  #else
    drawValueInBox(phFlowMixBox,UNAVAIL_FEATURE,TFT_FONT);
    drawValueInBox(phPlusMinusBox,UNAVAIL_FEATURE,TFT_FONT);
  #endif // USE_PH_MODULE
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void TFTIdleScreen::drawFlowData(TFTMenu* menuManager)
{
  ...
  rainStatusBox->draw(menuManager);
    ...
    drawValueInBox(flow1Box,UNAVAIL_FEATURE,TFT_FONT);
    drawValueInBox(flow2Box,UNAVAIL_FEATURE,TFT_FONT);
  #endif // USE_WATERFLOW_MODULE
    ...
    drawValueInBox(rainStatusBox,val,TFT_FONT);    
    
  #else // !USE_WINDOW_MANAGE_MODULE
  
    drawValueInBox(windSpeedBox,UNAVAIL_FEATURE,TFT_FONT);
    drawValueInBox(windDirectionBox,UNAVAIL_FEATURE,TFT_FONT);
    drawValueInBox(rainStatusBox,UNAVAIL_FEATURE,TFT_FONT);
...
void TFTIdleScreen::drawHeatSensorsData(TFTMenu* menuManager)
{
  ...
  #else
    drawValueInBox(heatBox1Sensor,UNAVAIL_FEATURE,TFT_FONT);
    drawValueInBox(heatBox1Air,UNAVAIL_FEATURE,TFT_FONT);
    
    drawValueInBox(heatBox2Sensor,UNAVAIL_FEATURE,TFT_FONT);
    drawValueInBox(heatBox2Air,UNAVAIL_FEATURE,TFT_FONT);
    
    drawValueInBox(heatBox3Sensor,UNAVAIL_FEATURE,TFT_FONT);
    drawValueInBox(heatBox3Air,UNAVAIL_FEATURE,TFT_FONT);
...
{
  const char* pressureCaption = "ДАВЛЕНИЕ: ";
  const char* mmrtCaption = "мм рт";
  const char* maxPressureCaption = "9999";
  ...
  int pressureCaptionLen = rusPrinter->textWidth(pressureCaption);
  int mmrtCaptionLen = rusPrinter->textWidth(mmrtCaption);
  int maxPressureCaptionLen = rusPrinter->textWidth(maxPressureCaption);


  const int SPACING = 20;
  const int PARTS_SPACING = 2;
  int left = curIconRightMargin - SPACING - (pressureCaptionLen + mmrtCaptionLen + maxPressureCaptionLen + PARTS_SPACING*2);
  ...
  rusPrinter->print(pressureCaption,curLeft,curTop,TFT_BACK_COLOR,INFO_BOX_CAPTION_COLOR);
  
  curLeft += pressureCaptionLen + PARTS_SPACING;

  // рисуем бокс с подложкой
  dc->fillRect(curLeft-1,curTop - 1 ,maxPressureCaptionLen+2,fontHeight + 2,INFO_BOX_CAPTION_COLOR);
  

 // рисуем значение давления
  String displayString = nowPressure.HasData() ? String(nowPressure.Value)  : "-";
  int strWidth = rusPrinter->textWidth(displayString.c_str());

  curLeft += (maxPressureCaptionLen - strWidth)/2;

  rusPrinter->print(displayString.c_str(),curLeft,curTop,INFO_BOX_CAPTION_COLOR,TFT_BACK_COLOR);

  curLeft += maxPressureCaptionLen + PARTS_SPACING;

// теперь рисуем "мм"

  rusPrinter->print(mmrtCaption,curLeft,curTop,TFT_BACK_COLOR,INFO_BOX_CAPTION_COLOR);  
  ...
  // рисуем бокс с подложкой
  int boxRight = fontWidth*MAX_PRESSURE_VALUE_LEN + 4;
  int savedPos = boxRight + curLeft;
  
  dc->fillRect(curLeft+2,curTop - 2 ,boxRight,fontHeight + 4,RED);//INFO_BOX_CAPTION_COLOR);


  // рисуем значение давления
  displayString = nowPressure.HasData() ? String(nowPressure.Value)  : String(random(999));//"-";
  strWidth = rusPrinter->textWidth(displayString.c_str());

  curLeft += (MAX_PRESSURE_VALUE_LEN*fontWidth - strWidth)/2 + 4;
...
int TFTIdleScreen::drawWiFiIcons(TFTMenu* menuManager, int curIconRightMargin, bool connectChanged, bool qualityChanged)
{
   ...
   if(qualityChanged)
   {
    ...
    int wifiTextLen = rus->textWidth("WIFI");
    initialLeft -= 14;
    initialLeft -= wifiTextLen;
    ...
    {           
       ...
       // dc->setBackColor(TFT_BACK_COLOR);
        ...
        {
          dc->fillRect(initialLeft,initialTop,wifiTextLen+6,signalHeight,INFO_BOX_CAPTION_COLOR);
        //  dc->setColor(SENSOR_BOX_FONT_COLOR);
        ...
        else
        {
         // dc->setColor(TFT_BACK_COLOR);
          //dc->setBackColor(TFT_BACK_COLOR);
          dc->fillRect(initialLeft,initialTop,wifiTextLen+6,signalHeight,TFT_BACK_COLOR);
          yield();
         // dc->setColor(INFO_BOX_CAPTION_COLOR);
          dc->drawRect(initialLeft,initialTop,wifiTextLen+6,signalHeight,INFO_BOX_CAPTION_COLOR);
        }
        
        menuManager->getRusPrinter()->print("WIFI",initialLeft+3,initialTop+5,connectedToRouter ? TFT_BACK_COLOR : INFO_BOX_CAPTION_COLOR,connectedToRouter ? TFT_BACK_COLOR : INFO_BOX_CAPTION_COLOR);
...
void TFTIdleScreen::update(TFTMenu* menuManager)
{
  ...
  if(millis() - unrTimer > UNR_TMR_INTERVAL)
  {
      ...
      if(!rCopy)
      {
        ...
        if(!unrVisible)
        {
            rusPrinter->print(RESTRICTED_FEATURES,10,10,TFT_BACK_COLOR,TFT_BACK_COLOR);
        }
        else
        {
          rusPrinter->print(RESTRICTED_FEATURES,10,10,TFT_BACK_COLOR,RED);
        }
...
void TFTUnrKScreen::draw(TFTMenu* menuManager)
{
  ...
  String line6 = F("Email: spywarrior(at)gmail.com");
  String line7 = F("Email: promavto(at)ntmp.ru");
  ...
  for(size_t i=0;i<vec.size();i++)
  {
    if(i > 0)
    {
      ...
      rusPrinter->print(vec[i],left,top,TFT_BACK_COLOR,fgColor); 
